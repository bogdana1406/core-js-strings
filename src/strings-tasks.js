...
function formatTime(minutes, seconds) {
  const minutesForm = Math.max(0, Math.floor(minutes));
  const secondsForm = Math.max(0, Math.floor(seconds));

  const formattedMinutes = String(minutesForm).padStart(2, '0');
  const formattedSeconds = String(secondsForm).padStart(2, '0');
...
function encodeToRot13(str) {
  function shiftChar(char, offset, isUpperCase) {
    const baseCharCode = isUpperCase ? 65 : 97;
    return String.fromCharCode(
      ((char.charCodeAt(0) - baseCharCode + offset) % 26) + baseCharCode
    );
  }

  return str.replace(/[a-zA-Z]/g, function rot13Shift(char) {
    const isUpperCase = char === char.toUpperCase();
    return shiftChar(char, 13, isUpperCase);
  });
}
